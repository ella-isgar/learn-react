...
function toggleList(
  ...
  return aList.map((e) => {
    if (e.id === artWordId) {
      // Don't chance the initial e value. Make a shallow
      // copy of e and modify only what variable you need
      // to with a new type
...
export default function BucketList() {
  // Potential recursive solution?: const clone = (initialList) => initialList.map(item => Array.isArray(item) ? clone(item) : item);
  ...
  function handleToggleYourList(artworkId: number, nextSeen: boolean) {
    ...
    const tmpList = yourList.map((e) => {
      if (e.id === artworkId) {
        e.seen = nextSeen; // Changes the value of the initial reference (both lists will be modified)
      }
      return e;
    });
    setYourList(tmpList);
